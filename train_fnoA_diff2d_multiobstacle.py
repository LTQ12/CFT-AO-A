"""
Train standard FNO2d on diffusion-multiobstacle 2D dataset with Direction-A wrapper:
boundary constructive extension E[g] + residual learning.
"""
...
def main(args):
    ...
    print(f"\nModel: FNO2d + A (diffusion-multiobstacle)")
    ...
    best_test = float("inf")
    print("\n--- Starting Training (FNO + A, diffusion-multiobstacle) ---")
    ...
    print("--- Training Finished (FNO + A, diffusion-multiobstacle) ---")
    if args.model_save_path:
        ...
        yn_path = os.path.join(model_dir, "diff2d_multiobs_fnoA_y_normalizer.pt")
        torch.save(yn_stats, yn_path)
        print(f"Model saved to {args.model_save_path}\nSaved u_normalizer stats to {yn_path}")


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Train FNO + A on diffusion-multiobstacle 2D dataset.")
    p.add_argument("--data_path", type=str, default="data/diffusion_multiobstacle2d_N128.pt")
    p.add_argument("--model_save_path", type=str, default="models/fnoA_diffusion_multiobstacle2d.pt")
